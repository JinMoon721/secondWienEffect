...
disE=df["EkBA"].to_numpy()[index] * sigma

mfpt=df["mfptBA"].to_numpy()[index]
mfptE=df["EmfptBA"].to_numpy()[index] * sigma


mk="p"
lb1=r"$\nu_{BA}/(1-\langle q_- \rangle)$"
...
plt.ylabel(r'$k_{\mathrm{diso}}(\xi)  \ / \ \mathrm{ns^{-1}}$', fontsize=fsize)
plt.xlim(0, 51)
