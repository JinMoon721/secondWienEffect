...
if figA:
    ...
    plt.fill_between(qx[1:], 100*(qn[1:]/qn[0]-1 + error*sigma), 100*(qn[1:]/qn[0] - error*sigma-1), alpha=ap, color=colors[ind])


    fx = qx[1:]
    fy = 100*(qn[1:]/qn[0]-1)
    popt, _ = curve_fit(objective, fx, fy)
    fity = objective(fx, popt[0], popt[1], popt[2], popt[3])
    plt.plot(fx, fity, '--')
    ...
    for i in range(len(qn)-1):
        error[i] = qn[i+1]/qn[0] * np.sqrt( (qnE[i+1]/qn[i+1])**2 + (qnE[0]/qn[0])**2)
    plt.fill_between(qx[1:], 100*(qn[1:]/qn[0]-1 + error*sigma), 100*(qn[1:]/qn[0]-1 - error*sigma), alpha=ap, color=colors[ind])
...
#    plt.ylabel(r'$\lambda^* \ \mathrm{or} \ \langle q_- \rangle^* $', fontsize=fsize)
