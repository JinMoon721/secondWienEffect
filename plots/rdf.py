...
if figA:
    ...
    plt.plot(ar,ay*kbT, linestyle='-',lw=3, marker='s', color=colors[0], mfc='w', markersize=0, label=r"$\mathrm{Li}$")
    mask = (hr > 5.3)
    hy[mask] = hy[mask]-0.05/kbT
    plt.plot(hr,hy*kbT, linestyle='-',lw=3, marker='o', color=colors[1], mfc='w', markersize=0, label=r"$\mathrm{Li^*}$")
...
plt.savefig('./figures/Lip.png' , dpi=300, bbox_inches="tight")
plt.show()
